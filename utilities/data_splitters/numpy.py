...
def get_label_count(labels, label):
    ...
    logger.debug(f'** in function: get_label_count')
    return len(np.nonzero(labels == label)[0])


def one_hot(labels, classes):
    """Apply One-Hot encoding to labels."""
    logger.debug(f'** in function: one_hot')
...
class NumPyDataSplitter(DataSplitter):
    ...
    def split(self, data: np.ndarray, num_collaborators: int) -> List[List[int]]:
        logger.debug(f'** in function: split')
        """Split the data."""
...
class EqualNumPyDataSplitter(NumPyDataSplitter):
    ...
    def __init__(self, shuffle=True, seed=0):
        ...
        """
        logger.debug(f'** in function: EqualNumPyDataSplitter:__init__')
        self.shuffle = shuffle
        self.seed = seed

    def split(self, data, num_collaborators):
        """Split the data."""
        logger.debug(f'** in function: EqualNumPyDataSplitter:split')
    ...
    def __init__(self, shuffle=True, seed=0):
        ...
        Args:
            ...
            seed(int): Random numbers generator seed.
                ...
                on each shard descriptor.
        """
        logger.debug(f'** in function: RandomNumPyDataSplitter:__init__')
        self.shuffle = shuffle
        self.seed = seed

    def split(self, data, num_collaborators):
        """Split the data."""
        logger.debug(f'** in function: RandomNumPyDataSplitter:split')
        np.random.seed(self.seed)
        idx = range(len(data))
        if self.shuffle:
            idx = np.random.permutation(idx)
...
class LogNormalNumPyDataSplitter(NumPyDataSplitter):
    ...
    def __init__(self, mu,
        ...
        Args:
            ...
            seed(int): Random numbers generator seed.
                ...
                on each shard descriptor.
        """
        logger.debug(f'** in function: LogNormalNumPyDataSplitter:__init__')
        self.mu = mu
        self.sigma = sigma
    ...
    def split(self, data, num_collaborators):
        ...
        """
        logger.debug(f'** in function: LogNormalNumPyDataSplitter:split')
        np.random.seed(self.seed)
...
class DirichletNumPyDataSplitter(NumPyDataSplitter):
    ...
    def __init__(self, alpha=0.5, min_samples_per_col=10, seed=0):
        ...
        Args:
            ...
            seed(int): Random numbers generator seed.
                ...
                on each shard descriptor.
        """
        logger.debug(f'** in function: DirichletNumPyDataSplitter:__init__')
        self.alpha = alpha
    ...
    def split(self, data, num_collaborators):
        """Split the data."""
        logger.debug(f'** in function: DirichletNumPyDataSplitter:split')
        np.random.seed(self.seed)
