...
def getfqdn_env(name: str = '') -> str:
    ...
    """
    logger.debug(f'** in function: getfqdn_env')
...
def is_fqdn(hostname: str) -> bool:
    ...
    logger.debug(f'** in function: is_fqdn')
...
def is_api_adress(address: str) -> bool:
    ...
    logger.debug(f'** in function: is_api_adress')
    try:
...
def add_log_level(level_name, level_num, method_name=None):
    ...
    """
    logger.debug(f'** in function: add_log_level')
    if not method_name:
        method_name = level_name.lower()

    def log_for_level(self, message, *args, **kwargs):
        logger.debug(f'** in function: log_for_level')
        if self.isEnabledFor(level_num):
            self._log(level_num, message, args, **kwargs)

    def log_to_root(message, *args, **kwargs):
        logger.debug(f'** in function: log_to_root')
...
def split_tensor_dict_into_floats_and_non_floats(tensor_dict):
    ...
    """
    logger.debug(f'** in function: split_tensor_dict_into_floats_and_non_floats')
    float_dict = {}
    non_float_dict = {}
...
def split_tensor_dict_by_types(tensor_dict, keep_types):
    ...
    """
    logger.debug(f'** in function: split_tensor_dict_by_types')
    keep_dict = {}
    holdout_dict = {}
    for k, v in tensor_dict.items():
...
def split_tensor_dict_for_holdouts(logger, tensor_dict,
    ...
    """
    logger.debug(f'** in function: split_tensor_dict_for_holdouts')
    # initialization
...
def validate_file_hash(file_path, expected_hash, chunk_size=8192):
    ...
    """
    logger.debug(f'** in function: validate_file_hash')
    h = hashlib.sha384()
...
def tqdm_report_hook():
    ...
    logger.debug(f'** in function: tqdm_report_hook')

    def report_hook(pbar, count, block_size, total_size):
        """Update progressbar."""
        logger.debug(f'** in function: report_hook')
...
def merge_configs(
    ...
    logger.debug(f'** in function: merge_configs')
...
def change_tags(tags, *, add_field=None, remove_field=None) -> Tuple[str, ...]:
    ...
    logger.debug(f'** in function: change_tags')
    tags = list(set(tags))
