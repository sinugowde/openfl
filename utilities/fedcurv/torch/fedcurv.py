...
def register_buffer(module: torch.nn.Module, name: str, value: torch.Tensor):
    ...
    Args:
        ...
        value: Buffer value
    """
    logger.debug(f'** in function: register_buffer')
...
def get_buffer(module, target):
    ...
    """
    logger.debug(f'** in function: get_buffer')
...
class FedCurv:
    ...
    def __init__(self, model: torch.nn.Module, importance: float):
        ...
        """
        logger.debug(f'** in function: FedCurv:__init__')
        self.importance = importance
        self._params = {}
        self._register_fisher_parameters(model)

    def _register_fisher_parameters(self, model):
        logger.debug(f'** in function: FedCurv:_register_fisher_parameters')
    ...
    def _update_params(self, model):
        logger.debug(f'** in function: FedCurv:_update_params')
    ...
    def _diag_fisher(self, model, data_loader, device):
        logger.debug(f'** in function: FedCurv:_diag_fisher')
        precision_matrices = {}
    ...
    def get_penalty(self, model):
        ...
        Returns:
            float: Penalty term.
        """
        logger.debug(f'** in function: FedCurv:get_penalty')
        penalty = 0
        if not self._params:
            return penalty
    ...
    def on_train_begin(self, model):
        ...
        """
        logger.debug(f'** in function: FedCurv:on_train_begin')
    ...
    def on_train_end(self, model: torch.nn.Module, data_loader, device):
        ...
        """
        logger.debug(f'** in function: FedCurv:on_train_end')
