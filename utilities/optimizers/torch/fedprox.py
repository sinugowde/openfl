...
class FedProxOptimizer(Optimizer):
    ...
    def __init__(self,
        ...
        logger.debug(f'** in function: FedProxOptimizer:__init__')
        if momentum < 0.0:
    ...
    def __setstate__(self, state):
        """Set optimizer state."""
        logger.debug(f'** in function: FedProxOptimizer:__setstate__')
    ...
    def step(self, closure=None):
        ...
        """
        logger.debug(f'** in function: FedProxOptimizer:step')
        loss = None
    ...
    def set_old_weights(self, old_weights):
        ...
        logger.debug(f'** in function: FedProxOptimizer:set_old_weights')
...
class FedProxAdam(Optimizer):
    ...
    def __init__(self, params, mu=0, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        """Initialize."""
        logger.debug(f'** in function: FedProxAdam:__init__')
        if not 0.0 <= lr:
            raise ValueError(f'Invalid learning rate: {lr}')
        if not 0.0 <= eps:
    ...
    def __setstate__(self, state):
        """Set optimizer state."""
        logger.debug(f'** in function: FedProxAdam:__setstate__')
    ...
    def set_old_weights(self, old_weights):
        """Set the global weights parameter to `old_weights` value."""
        logger.debug(f'** in function: FedProxAdam:set_old_weights')
        for param_group in self.param_groups:
            param_group['w_old'] = old_weights
    ...
    def step(self, closure=None):
        ...
        Args:
            closure (callable, optional): A closure that reevaluates the model
                and returns the loss.
        """
        logger.debug(f'** in function: FedProxAdam:step')
        loss = None
        if closure is not None:
    ...
    def adam(self, params,
        ...
        logger.debug(f'** in function: FedProxAdam:adam')
