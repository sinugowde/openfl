...
class FederatedFastEstimator:
    ...
    def __init__(self, estimator, override_config: dict = None, **kwargs):
        ...
        self.logger.debug(f'** in function: FederatedFastEstimator:__init__')
        fx.init(**kwargs)
        if override_config:
    ...
    def fit(self):
        """Run the estimator."""
        self.logger.debug(f'** in function: FederatedFastEstimator:fit')
...
def split_data(train, eva, test, rank, collaborator_count):
    ...
    logger1.debug(f'** in function: Fsplit_data')
