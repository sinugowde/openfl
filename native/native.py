...
def setup_plan(log_level='CRITICAL'):
    ...
    Returns:
        plan : Plan object
    """
    logger.debug(f'** in function: setup_plan')
...
def flatten(config, return_complete=False):
    ...
    logger.debug(f'** in function: flatten')
...
def update_plan(override_config):
    ...
    Returns:
        None
    """
    logger.debug(f'** in function: update_plan')
    plan = setup_plan()
...
def unflatten(config, separator='.'):
    ...
    logger.debug(f'** in function: unflatten')
...
def init(workspace_template: str = 'default', log_level: str = 'INFO',
    ...
    logger.debug(f'** in function: init')
...
def create_collaborator(plan, name, model, aggregator):
    ...
    """
    logger.debug(f'** in function: create_collaborator')
    plan = copy(plan)
...
def run_experiment(collaborator_dict: dict, override_config: dict = None):
    ...
    """
    logger.debug(f'** in function: run_experiment')
...
def get_plan(indent=4, sort_keys=True):
    ...
    logger.debug(f'** in function: get_plan')
    import json
    plan = setup_plan()
