...
def workspace(context):
    ...
    logger.debug(f'** in function: workspace')
    context.obj['group'] = 'workspace'


def create_dirs(prefix):
    """Create workspace directories."""
    logger.debug(f'** in function: create_dirs')
...
def create_temp(prefix, template):
    ...
    logger.debug(f'** in function: create_temp')
...
def get_templates():
    ...
    logger.debug(f'** in function: get_templates')
    from openfl.interface.cli_helper import WORKSPACE
...
def create_(prefix, template):
    ...
    logger.debug(f'** in function: create_')
...
def create(prefix, template):
    ...
    logger.debug(f'** in function: create')
...
def export_(pip_install_options: Tuple[str]):
    ...
    logger.debug(f'** in function: export_')
...
def import_(archive):
    ...
    logger.debug(f'** in function: import_')
...
def certify_():
    ...
    logger.debug(f'** in function: certify_')
    certify()


def certify():
    """Create certificate authority for federation."""
    logger.debug(f'** in function: certify')
...
def _get_requirements_dict(txtfile):
    logger.debug(f'** in function: _get_requirements_dict')
...
def _get_dir_hash(path):
    logger.debug(f'** in function: _get_dir_hash')
...
def dockerize_(context, base_image, save):
    ...
    """
    logger.debug(f'** in function: dockerize_')
    import docker
    import sys
    from shutil import copyfile
...
def graminize_(context, signing_key: Path, pip_install_options: Tuple[str],
    ...
    """
    logger.debug(f'** in function: graminize_')
    def open_pipe(command: str):
        logger.debug(f'** in function: open_pipe')
        echo(f'\n 📦 Executing command:\n{command}\n')
...
def apply_template_plan(prefix, template):
    ...
    """
    logger.debug(f'** in function: apply_template_plan')
