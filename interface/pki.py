...
def pki(context):
    """Manage Step-ca PKI."""
    logger.debug(f'** in function: pki')
...
def run(ca_path):
    """Run CA server."""
    logger.debug(f'** in function: run')
...
def install_(ca_path, password, ca_url):
    ...
    logger.debug(f'** in function: install_')
    ca_path = Path(ca_path).absolute()
...
def uninstall(ca_path):
    """Remove step-CA."""
    logger.debug(f'** in function: uninstall')
    ca_path = Path(ca_path).absolute()
    remove_ca(ca_path)
...
def get_token_(name, ca_url, ca_path):
    ...
    """
    logger.debug(f'** in function: get_token_')
    ca_path = Path(ca_path).absolute()
