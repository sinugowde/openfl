...
def pretty(o):
    ...
    logger.debug(f'** in function: pretty')
...
def tree(path):
    ...
    logger.debug(f'** in function: tree')
    echo(f'+ {path}')
...
def print_tree(dir_path: Path, level: int = -1,
    ...
    logger.debug(f'** in function: print_tree')
    space = '    '
    branch = '│   '
    tee = '├── '
    last = '└── '
    ...
    def inner(dir_path: Path, prefix: str = '', level=-1):
        logger.debug(f'** in function: inner')
...
def copytree(src, dst, symlinks=False, ignore=None,
    ...
    logger.debug(f'** in function: copytree')
    import os
    import shutil
    ...
    def _copytree():
        logger.debug(f'** in function: _copytree')
...
def get_workspace_parameter(name):
    ...
    logger.debug(f'** in function: get_workspace_parameter')
...
def check_varenv(env: str = '', args: dict = None):
    ...
    logger.debug(f'** in function: check_varenv')
    if args is None:
        args = {}
...
def get_fx_path(curr_path=''):
    ...
    logger.debug(f'** in function: get_fx_path')
    import re
    import os
...
def remove_line_from_file(pkg, filename):
    ...
    logger.debug(f'** in function: remove_line_from_file')
...
def replace_line_in_file(line, line_num_to_replace, filename):
    ...
    logger.debug(f'** in function: replace_line_in_file')
    with open(filename, 'r+') as f:
        d = f.readlines()
        f.seek(0)
