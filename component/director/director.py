...
class Director:
    ...
    def __init__(
        ...
        logger.debug(f'** in function: Director:__init__')
    ...
    def acknowledge_shard(self, shard_info: dict) -> bool:
        ...
        logger.debug(f'** in function: Director:acknowledge_shard')
        is_accepted = False
    ...
    async def set_new_experiment(
        ...
        """Set new experiment."""
        logger.debug(f'** in function: Director:set_new_experiment')
    ...
    def get_trained_model(self, experiment_name: str, caller: str, model_type: str):
        """Get trained model."""
        logger.debug(f'** in function: Director:get_trained_model')
    ...
    async def stream_metrics(self, experiment_name: str, caller: str):
        ...
        """
        logger.debug(f'** in function: Director:stream_metrics')
        if (experiment_name not in self.experiments_registry
                or caller not in self.experiments_registry[experiment_name].users):
            raise Exception(
    ...
    def remove_experiment_data(self, experiment_name: str, caller: str):
        ...
        logger.debug(f'** in function: Director:remove_experiment_data')
    ...
    def set_experiment_failed(self, *, experiment_name: str, collaborator_name: str):
        ...
        logger.debug(f'** in function: Director:set_experiment_failed')
        if experiment_name in self.experiments_registry:
            aggregator = self.experiments_registry[experiment_name].aggregator
    ...
    def update_envoy_status(
        ...
        logger.debug(f'** in function: Director:update_envoy_status')
    ...
    def get_envoys(self) -> list:
        ...
        logger.debug(f'** in function: Director:get_envoys')
    ...
    def get_experiments_list(self, caller: str) -> list:
        ...
        logger.debug(f'** in function: Director:get_experiments_list')
    ...
    def get_experiment_description(self, caller: str, name: str) -> dict:
        ...
        logger.debug(f'** in function: Director:get_experiment_description')
    ...
    async def start_experiment_execution_loop(self):
        ...
        logger.debug(f'** in function: Director:start_experiment_execution_loop')
        while True:
...
def _get_model_download_statuses(experiment) -> List[dict]:
    logger.debug(f'** in function: Director:_get_model_download_statuses')
...
def _get_experiment_progress(experiment) -> Union[float, None]:
    logger.debug(f'** in function: Director:_get_experiment_progress')
...
def _get_experiment_tasks(experiment) -> List[dict]:
    logger.debug(f'** in function: Director:_get_experiment_tasks')
    return [{
        'name': task['function'],
...
def _get_experiment_collaborators(experiment) -> List[dict]:
    logger.debug(f'** in function: Director:_get_experiment_collaborators')
    return [{
        'name': name,
        'status': 'pending_mock',
        'progress': 0.0,
